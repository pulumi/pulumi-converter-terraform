all_file_paths    = notImplemented("fileset(var.base_dir,\"**\")")
static_file_paths = notImplemented("toset([\nforpinlocal.all_file_paths:p\niflength(p)<length(var.template_file_suffix)||substr(p,length(p)-length(var.template_file_suffix),length(var.template_file_suffix))!=var.template_file_suffix\n])")
template_file_paths = { for p in all_file_paths : invoke("std:index:substr", {
  input  = p
  length = 0
  offset = length(p) - length(templateFileSuffix)
}).result => p if !notImplemented("contains(local.static_file_paths,p)") }
template_file_contents  = { for p, sp in template_file_paths : p => notImplemented("templatefile(\"$${var.base_dir}/$${sp}\",var.template_vars)") }
static_file_local_paths = { for p in static_file_paths : p => "${baseDir}/${p}" }
output_file_paths       = notImplemented("setunion(keys(local.template_file_paths),local.static_file_paths)")
file_suffix_matches     = { for p in output_file_paths : p => notImplemented("regexall(\"\\\\.[^\\\\.]+\\\\z\",p)") }
file_suffixes           = { for p, ms in file_suffix_matches : p => length(ms) > 0 ? ms[0] : "" }
file_types              = { for p in output_file_paths : p => notImplemented("lookup(var.file_types,local.file_suffixes[p],var.default_file_type)") }
