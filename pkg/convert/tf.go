...
func translateModuleSourceCode(
	...
	for _, item := range items {
		if item.moduleCall != nil {
			...
			if source, isRemote := moduleFromRemoteRegistry(moduleCall); isRemote {
				key := fmt.Sprintf("%s-%s", source.Package.Namespace, source.Package.Name)
	...
	for _, item := range items {
		...
		if topLevelModule {
			...
			moduleNames := make([]string, 0, len(sandboxedModules))
			for name := range sandboxedModules {
				moduleNames = append(moduleNames, name)
			}
			slices.Sort(providerBlockNames)

			for _, name := range moduleNames {
				sandboxModule := sandboxedModules[name]
